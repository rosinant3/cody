...
exports.up = function (knex) {
    return knex.schema.createTable('userBlockList', function (t) {
      t.increments('id').primary();
      t.integer('user').unsigned().collate('utf8_unicode_ci').notNullable();
      t.integer('blocks').unsigned().collate('utf8_unicode_ci').notNullable();
      t.timestamps(false, true);
      t.foreign('user').references('id').inTable('user');
      t.foreign('blocks').references('id').inTable('user');
    })
  }
  exports.down = function (knex) {
    return knex.schema.dropTableIfExists('userBlockList')
  }
